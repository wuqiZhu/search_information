...
static int __init dht11_init(void)
{
	...
	{		
		...
		/* 设置DHT11 GPIO引脚的初始状态: output 1 */
		/* 设置DHT11 GPIO引脚的初始状态: input（由驱动动态切换） */	
        err = gpio_request(gpios[i].gpio, gpios[i].name);
		if (err) {
			printk("gpio_request failed for %s\n", gpios[i].name);
		} else {
			gpio_direction_input(gpios[i].gpio);   // 改为输入模式
	...
	/* 注册file_operations 	*/
	major = register_chrdev(0, "dht11_new", &dht11_drv);  /* /dev/gpio_desc */

	gpio_class = class_create(THIS_MODULE, "dht11_new_class");
	if (IS_ERR(gpio_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "dht11_new");
		return PTR_ERR(gpio_class);
	}

	device_create(gpio_class, NULL, MKDEV(major, 0), NULL, "dht11_new"); /* /dev/mydht11 */
