...
class NotificationDispatcher:
    ...
    def dispatch_all(
        ...
        # Invest API
        if self.config.get("INVEST_API_URL"):
            results["invest"] = self._send_invest(
                report_data, report_type, proxy_url, rss_items, rss_new_items, ai_analysis
            )
    ...
    def _send_invest(
        ...
        """发送新闻数据到 invest 项目的 API"""
        return send_to_invest_api(
            api_url=self.config["INVEST_API_URL"],
            report_data=report_data,
            report_type=report_type,
            api_key=self.config.get("INVEST_API_KEY", ""),
            proxy_url=proxy_url,
            rss_items=rss_items,
    ...
    def _send_rss_dingtalk(
        ...
        for i, webhook_url in enumerate(webhooks):
            ...
            try:
                ...
                for batch_idx, batch_content in enumerate(batches):
                    title = f"📰 RSS 通知 {f'({batch_idx + 1}/{len(batches)})' if len(batches) > 1 else ''}"
                    payload = {
                        "msgtype": "markdown",
                        "markdown": {
                            "title": title,
