...
def send_to_invest_api(
    ...
    *,
    ...
    """
    发送新闻数据到 invest 项目的 API 接口

    Args:
        api_url: invest API 地址（如 http://localhost:5000/api/news-sentiment）
        report_data: 报告数据
        report_type: 报告类型
        api_key: API 密钥（可选）
        proxy_url: 代理 URL（可选）
    ...
    Returns:
        bool: 发送是否成功
    """
    log_prefix = f"invest{account_label}" if account_label else "invest"
    ...
    for stat in report_data.get("stats", []):
        ...
        for title_data in stat.get("titles", []):
            news_items.append({
                ...
                "hotness": title_data.get("hotness", 0),
            })

    for source in report_data.get("new_titles", []):
        platform = source.get("source", "")
        for title_data in source.get("titles", []):
            news_items.append({
                "title": title_data.get("title", ""),
                "platform": platform,
                "rank": title_data.get("rank", 0),
    ...
    if ai_analysis and getattr(ai_analysis, "success", False):
        ai_summary = getattr(ai_analysis, "summary", None) or getattr(ai_analysis, "analysis", None)
    ...
    try:
        print(f"{log_prefix}发送 {len(news_items)} 条新闻到 invest API [{report_type}]")
        response = requests.post(
            api_url, headers=headers, json=payload, proxies=proxies, timeout=30
        )
        if response.status_code >= 200 and response.status_code < 300:
            print(f"{log_prefix}发送成功 [{report_type}]")
            return True
        else:
            print(
                f"{log_prefix}发送失败 [{report_type}]，状态码：{response.status_code}, 响应: {response.text}"
            )
            return False
    except Exception as e:
        print(f"{log_prefix}发送出错 [{report_type}]：{e}")
        return False
    ...
    *,
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"TrendRadar 通知 - {report_type}",
                "text": batch_content,
            },
        }
