...
class AIAnalyzer:
    ...
    def _call_ai(self, user_prompt: str) -> str:
        """调用 AI API（使用 LiteLLM），带响应缓存"""
        messages = []
        if self.system_prompt:
            messages.append({"role": "system", "content": self.system_prompt})
        messages.append({"role": "user", "content": user_prompt})

        # === AI 响应缓存：相同输入直接返回缓存结果 ===
        try:
            from trendradar.ai.response_cache import cached_chat
            response = cached_chat(self.client, messages)
        except Exception:
            # 缓存失败时回退到直接调用
            response = self.client.chat(messages)

        # === 训练数据缓冲池：被动收集 API 调用记录 ===
        try:
            ...
            save_to_buffer(
                ...
                metadata={
                    ...
                    "system_prompt": self.system_prompt[:300] if self.system_prompt else "",
                    "mode": "trendradar_ai_analysis",
                }
            )
        except Exception:
            # 缓冲池失败不影响主流程
