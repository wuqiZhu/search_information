...
def _load_notification_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        "SCHEDULED_PUSH": notification.get("scheduled_push", {}),
...
def _load_webhook_config(config_data: Dict) -> Dict:
    ...
    return {
        ...
        # Invest API
        "INVEST_API_URL": _get_env_str("INVEST_API_URL") or invest.get("api_url", ""),
        "INVEST_API_KEY": _get_env_str("INVEST_API_KEY") or invest.get("api_key", ""),
    }
...
def _print_notification_sources(config: Dict) -> None:
    ...
    if config.get("INVEST_API_URL"):
        source = "环境变量" if os.environ.get("INVEST_API_URL") else "配置文件"
        notification_sources.append(f"Invest API({source})")
