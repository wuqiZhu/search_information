...
class NewsAnalyzer:
    ...
    def _send_notification_if_needed(
        ...
        # 定时推送模式下，跳过实时推送
        scheduled_config = cfg.get("SCHEDULED_PUSH", {})
    ...
    def _check_scheduled_push(self) -> None:
        """定时推送检查 - 每天固定时间推送"""
        try:
            scheduled_config = self.ctx.config.get("SCHEDULED_PUSH", {})
            ...
            scheduled_times = [t for t in scheduled_config.get("times", ["09:00", "13:00", "18:00", "22:00"]) if t]
    ...
    def _send_text_summary(self, dispatcher, summary_data: dict, date_str: str, time_str: str) -> None:
        """发送文字版总结（优先使用AI生成大白话解读）"""
        period = "晨报" if time_str < "12:00" else ("午报" if time_str < "17:00" else "晚报")

        # 尝试AI生成简报
        ai_briefing = self._generate_ai_briefing(summary_data, date_str, period)
        if ai_briefing:
            markdown = ai_briefing
            print(f"[定时推送] 使用AI简报")
        else:
            # 回退到标题分类列表
            print(f"[定时推送] AI简报不可用，回退到标题列表")
    ...
    def _generate_ai_briefing(self, summary_data: dict, date_str: str, period: str) -> Optional[str]:
        """调用AI生成大白话解读摘要（事件→标志→代表）"""
        try:
            # 收集所有新闻条目
            all_items = []
            for category, items in summary_data.items():
                for item in items:
                    source = item.get("source", item.get("feed_name", ""))
                    text = f"【{category}】{item.get('title', '')}"
                    if source:
                        text += f" ({source})"
            ...
            # 加载AI简报提示词
            prompt_path = Path(__file__).parent.parent / "config" / "ai_briefing_prompt.txt"
            if not prompt_path.exists():
                print(f"[AI简报] 提示词文件不存在: {prompt_path}")
                return None
            ...
            if "[system]" in content and "[user]" in content:
                ...
                # 提取 system 内容（去掉 [system] 行）
                system_lines = []
                for line in system_part.split("\n"):
            ...
            # 替换模板变量
            system_prompt = system_prompt.replace("{period}", period).replace("{date_str}", date_str)
            ...
            # 填充用户消息模板
            if user_template:
                user_message = user_template.replace("{period}", period).replace("{date_str}", date_str).replace("{news_content}", news_text)
            else:
                user_message = f"今天是 {date_str}，{period}时段。\n\n以下是今天收集到的新闻，请挑选最重要的信号解读：\n\n{news_text}"
    ...
    def _run_sentinel(self, results, rss_items, rss_new_items, raw_rss_items):
        """运行哨兵：对新闻分类/情绪/相关度打分"""
        try:
            import sys as _sys
            _sys.path.insert(0, str(Path(__file__).parent.parent / "scripts"))
            from classifier_server import get_classifier

            models_dir = "/app/models"
            if not Path(models_dir).exists():
                models_dir = str(Path(__file__).parent.parent.parent / "models")
            clf = get_classifier(models_dir)
            if "sentiment" not in clf.sessions:
                return results, rss_items, rss_new_items, raw_rss_items
            ...
            for pid, val in results.items():
                ...
                for title, data in val.items():
                    if isinstance(title, str) and title.strip() and isinstance(data, dict):
                        all_titles.append(title)
                        title_refs.append((val, title))

            if not all_titles:
                return results, rss_items, rss_new_items, raw_rss_items

            sentiments = clf.predict("sentiment", all_titles)
            categories = clf.predict("category", all_titles)
            relevances = clf.predict("relevance", all_titles)

            kept = 0
            for i, (parent, title) in enumerate(title_refs):
                sent = sentiments[i]["label"]
                cat = categories[i]["label"]
                rel = relevances[i]["label"]
                if isinstance(parent, dict) and title in parent:
                    if isinstance(parent[title], dict):
                        parent[title]["sentiment"] = sent
                        parent[title]["category"] = cat
                        parent[title]["relevance"] = rel
                    kept += 1

            print(f"[哨兵] 完成: 已标记{len(all_titles)}条新闻 (情绪/分类/相关度)")
        except Exception as e:
            import traceback
            print(f"[哨兵] 错误: {e}")
            traceback.print_exc()
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # 哨兵分类处理
            try:
                results, rss_items, rss_new_items, raw_rss_items = self._run_sentinel(
                    results, rss_items, rss_new_items, raw_rss_items
                )
            except Exception as e:
                print(f"[哨兵] 跳过 (不影响主流程): {e}")
